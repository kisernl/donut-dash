...
function Car() {
  ...
  // Create car body (black)
  const body = new THREE.Mesh(
    new THREE.BoxGeometry(60 * zoom, 30 * zoom, 15 * zoom),
    new THREE.MeshPhongMaterial({ color: 0x000000, flatShading: true }) // Black body
  );
  ...
  // Create car cabin (white with blue stripe)
  const cabinGeometry = new THREE.BoxGeometry(33 * zoom, 24 * zoom, 12 * zoom);
  const whiteMaterial = new THREE.MeshPhongMaterial({
    color: 0xffffff,
    flatShading: true,
  });
  const blueMaterial = new THREE.MeshPhongMaterial({
    color: 0x0000ff, // Bright blue for the stripe
    flatShading: true,
  });

  // Create materials array with blue stripe on top
  const cabinMaterials = [
    whiteMaterial,
    whiteMaterial,
    whiteMaterial,
    whiteMaterial,
    blueMaterial, // Top face will be blue
    whiteMaterial, // Bottom face remains white
  ];

  const cabin = new THREE.Mesh(cabinGeometry, cabinMaterials);
  cabin.position.x = 6 * zoom;
  cabin.position.z = 25.5 * zoom;
  cabin.castShadow = true;
  cabin.receiveShadow = true;
...
function Chicken() {
  ...
  const body = new THREE.Mesh(
    ...
    new THREE.MeshPhongMaterial({ color: 0xf5a623, flatShading: true }) // Donut color (light orange)
  );
  body.position.z = 10 * zoom;
  body.castShadow = true;
  body.receiveShadow = true;
  ...
  // Sprinkles (randomly placed small boxes)
  //   for (let i = 0; i < 20; i++) {
  //     const sprinkle = new THREE.Mesh(
  //       new THREE.BoxGeometry(0.5 * zoom, 0.5 * zoom, 1 * zoom),
  //       new THREE.MeshPhongMaterial({ color: Math.random() * 0xffffff }) // Random sprinkle colors
  //     );

  // Randomly position the sprinkles on the glaze
  // const angle = Math.random() * Math.PI * 2;
  // const radius = 10.5 * zoom + Math.random() * 1 * zoom;
  // sprinkle.position.set(
  //   Math.cos(angle) * radius,
  //   Math.sin(angle) * radius,
  //   10.2 * zoom + Math.random() * 0.2 * zoom
  // );
  ...
  if (stepStartTimestamp) {
    ...
    switch (moves[0]) {
      case "forward": {
        ...
        chicken.position.y = positionY; // initial position for chicken is 0
      ...
      case "left": {
        ...
        chicken.position.x = positionX; // initial chicken position set to 0
        chicken.position.z = jumpDeltaDistance;
        break;
      }
      case "right": {
...
requestAnimationFrame(animate);
// next code line   -- https://github.com/kaizhelam/Crossy-Road-Game/blob/main/script.js
